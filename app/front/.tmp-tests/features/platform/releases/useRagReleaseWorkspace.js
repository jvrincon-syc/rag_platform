...
import { buildRelease, createReleaseDraft, getConfiguration, getRelease, getReleaseBuildStatus, listAllCorpusSnapshots, listAllReleases, listAllVariants, publishRelease, retireRelease, validateRelease, } from "../platformApi.js";
...
export function useRagReleaseWorkspace() {
    ...
    const [seededStatus, setSeededStatus] = useState(null);
    ...
    const [buildEmbeddingRuntime, setBuildEmbeddingRuntime] = useState(null);
    ...
    const buildPoll = usePollingLoop({
        resourceId: buildTarget ? `${buildTarget.releaseId}:${buildTarget.nonce}` : null,
        intervalMs: 2500,
        fetchOnce: (signal) => getReleaseBuildStatus(buildReleaseId, { signal }),
        // `null` (aún sin job) no es terminal: se sigue consultando hasta encontrarlo.
        isTerminal: (status) => status !== null && (status.state === "succeeded" || status.state === "failed"),
    });
    ...
    const build = useCallback(async () => {
        ...
        try {
            ...
            const accepted = await idempotent.run(`build:${releaseId}:${runtime ?? "global"}`, (options) => buildRelease(releaseId, runtime, options));
            setNotice({
                tone: "info",
    ...
    useEffect(() => {
        ...
        if (succeeded) {
            ...
            setNotice({
                tone: "warning",
                message: "El seguimiento del build venció sin estado terminal. El build sigue del lado del servidor; usa Actualizar para reconsultar.",
            });
        }
